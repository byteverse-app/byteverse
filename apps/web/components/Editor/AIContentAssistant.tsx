...
interface AIContentAssistantProps {
  ...
  onApplyTransformedText?: (original: string, transformed: string) => void;
  onClose: () => void;
}
    ...
    try {
      ...
      if (actionType === 'generate-quiz' || actionType === 'generate-section') {
        ...
        if (response.ok) {
          ...
          if (result && onApplyTransformedText) {
            onApplyTransformedText(selectedText, result);
            onClose();
          } else if (selectedElement && selectedText && result) {
            alert(`AI ${actionType} completed. Copy the result from the assistant panel if needed.`);
          }
        }
      }
    } catch (error) {
