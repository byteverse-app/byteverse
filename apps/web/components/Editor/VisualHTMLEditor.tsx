...
export default function VisualHTMLEditor({
  ...
  const applyTransformedText = useCallback((original: string, transformed: string) => {
    if (!currentStage || !original) return;
    ...
    if (content.introduction?.includes(original)) {
      handleUpdateStage({
        ...currentStage,
        content: { ...content, introduction: replaceIn(content.introduction) },
      });
      return;
    }
    if (content.summary?.includes(original)) {
      handleUpdateStage({
        ...currentStage,
        content: { ...content, summary: replaceIn(content.summary) },
      });
      return;
    }
    const sectionIndex = content.sections.findIndex(
      (s) => (typeof s.content === 'string' ? s.content : '').includes(original)
    );
    if (sectionIndex >= 0) {
      ...
      sections[sectionIndex] = {
        ...section,
        content: replaceIn(typeof section.content === 'string' ? section.content : ''),
      };
      handleUpdateStage({
        ...currentStage,
        ...
        >
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-6">
              ...
              <SectionRegenerator
                stage={currentStage}
                stageIndex={courseData.course.stages.findIndex((s) => s.id === selectedStageId)}
                config={courseConfig}
                provider="together"
                onRegenerate={(_stageIndex, newContent) => {
                  handleUpdateStage({
                    ...currentStage,
                    ...newContent,
                  });
        ...
        <div className="w-80 bg-bg2 border-l border-border overflow-y-auto">
          ...
          {showAIAssistant && (
            <AIContentAssistant
              ...
              onApplyTransformedText={applyTransformedText}
              onClose={() => {
