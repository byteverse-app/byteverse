...
function DashboardContent() {
  ...
  const { featuredCourses, recentCourses, deleteCourse, importMessage, loading } = useCourses();
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <EngagementBanner />
        {importMessage && (
          <div className="mb-6 neu-card px-4 py-3 text-sm text-accent1 border border-accent1/20">
            {importMessage}
          </div>
        )}
        {loading ? (
          <div className="text-center text-text-secondary py-20">Loading your projects…</div>
        ) : activeTab === 'all' && (
          <>
            ...
            {featuredCourses.length === 0 && recentCourses.length === 0 && (
              <div className="neu-empty-state max-w-2xl mx-auto mt-8">
                ...
                <h2 className="text-3xl font-syne font-bold mb-4 text-text-primary">
                  Create Your First Microlearning Course
                </h2>
                <p className="text-lg text-text-secondary mb-10 max-w-lg mx-auto leading-relaxed">
                  ByteVerse helps you create engaging, bite-sized learning experiences. Upload your content, 
