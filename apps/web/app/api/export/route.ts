...
import { awardAchievement, updateStreakAndScore } from '@/lib/access/gamification';
...
Generated by ByteVerse - AI-Powered Microlearning Course Builder
`;

    archive.append(readmeContent, { name: 'README.md' });

    awardAchievement(authResult.ctx.user.id, 'exporter').catch(() => {});
    updateStreakAndScore(authResult.ctx.user.id, { exports: 1 }).catch(() => {});
    ...
    return internalError(error, 'export');
  }
}
