  ...
  const validateInvite = useCallback(async (code: string, advanceOnSuccess = false) => {
    ...
    try {
      ...
      if (isValid) {
        setInviteMessage(
          data.type === 'referral'
            ? `Referral invite — +${data.bonusCredits ?? 2} bonus credits on signup`
            : `Invite accepted — ${data.grantsTier ?? 'default'} tier`
        );
        if (advanceOnSuccess) {
    ...
    <AuthShell
      ...
      subtitle={
        inviteGatePassed
          ...
          : 'ByteVerse is invite-only. Enter your invite or referral code to continue.'
      }
      footer={
