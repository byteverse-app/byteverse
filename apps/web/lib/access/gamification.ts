...
export async function addCreatorScore(userId: string, points: number): Promise<void> {
  if (points <= 0) return;
  const supabase = createServiceRoleSupabaseClient();
  const { data: profile } = await supabase
    .from('profiles')
  ...
  await supabase
    .from('profiles')
    .update({ creator_score: (profile?.creator_score ?? 0) + points })
    .eq('id', userId);
}
...
export async function processReferralRewards(refereeId: string) {
  ...
  const referrerPoints = 15;
  const refereePoints = 5;

  await supabase.from('referral_events').insert({
    referrer_id: referee.referred_by,
    referee_id: refereeId,
    event: 'first_course_complete',
    credits_awarded: referrerPoints,
  });

  await addCreatorScore(referee.referred_by, referrerPoints);
  await addCreatorScore(refereeId, refereePoints);
}
...
export async function getReferralStats(userId: string) {
  ...
  const pointsEarned = (points ?? []).reduce((sum, e) => sum + (e.credits_awarded ?? 0), 0);
