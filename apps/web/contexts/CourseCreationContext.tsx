...
export function CourseCreationProvider({ children }: { children: React.ReactNode }) {
  const { user, updateCourse, getCourseState } = useCourses();
  ...
  const saveToStorage = useCallback(() => {
    ...
    if (user && currentCourseId) {
      updateCourse(currentCourseId, { state }).catch(console.error);
      return;
    }
  ...
  }, [state, isHydrated, currentCourseId, getStorageKey, user, updateCourse]);

  // Load from localStorage on mount when not using Supabase
  useEffect(() => {
  ...
  // Persist state changes (Supabase or localStorage)
  useEffect(() => {
  ...
  }, [state, isHydrated, saveToStorage, user, currentCourseId]);
  ...
  const loadStateForCourse = useCallback((courseId: string | null, courseState?: CourseCreationState) => {
    ...
    if (courseState) {
      ...
      const fromContext = getCourseState(courseId);
      if (fromContext) {
        setState({
          ...createInitialState(),
          ...fromContext,
          uploadedFiles: Array.isArray(fromContext.uploadedFiles) ? fromContext.uploadedFiles : [],
          chatHistory: Array.isArray(fromContext.chatHistory) ? fromContext.chatHistory : [],
          mediaAssets: Array.isArray(fromContext.mediaAssets) ? fromContext.mediaAssets : [],
          lastUpdated: Date.now(),
        });
        return;
      }
  ...
  }, [isHydrated, getStorageKey, getCourseState, user]);
