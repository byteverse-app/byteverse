...
const BYTENIMBUS_FEATURES = {
  ai: [
    ...
    { name: 'Assessment Builder', color: '#8B5CF6' },
    { name: 'Learning Optimizer', color: '#A855F7' }
  ],
  framework: [
    { name: 'ADDIE Framework', color: '#3B82F6' },
    { name: 'Bloom\'s Taxonomy', color: '#1D4ED8' },
    { name: 'Adult Learning', color: '#2563EB' },
    { name: 'Cognitive Load', color: '#1E40AF' }
  ],
  features: [
    { name: 'SCORM Export', color: '#10B981' },
    { name: 'Learning Analytics', color: '#059669' },
    { name: 'Team Collaboration', color: '#047857' },
    { name: 'LMS Integration', color: '#065F46' }
  ],
  content: [
    { name: 'Microlearning', color: '#F59E0B' },
    { name: 'Interactive Media', color: '#D97706' },
    { name: 'Gamification', color: '#B45309' },
    { name: 'Spaced Repetition', color: '#92400E' }
  ]
};
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    this.textLabels = [];
    this.camera = null; // Will be set by createBallpit
    this.#S();
  ...
  createTextLabels() {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      // Clear and redraw with better spacing
      context.clearRect(0, 0, canvas.width, canvas.height);
      lines.forEach((line, lineIndex) => {
        context.fillText(line, canvas.width / 2, canvas.height / 2 + (lineIndex - lines.length / 2) * 22);
      });
      ...
      const spriteMaterial = new spriteMat({ 
        map: texture,
        transparent: true,
        alphaTest: 0.1
      });
      ...
      const sprite = new spriteClass(spriteMaterial);
      sprite.scale.set(4, 2, 1);
      sprite.userData = { ballIndex: idx, feature: feature.name };
  ...
  update(e) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      // Update text label position with better offset
      if (this.textLabels[idx]) {
        this.textLabels[idx].position.copy(U.position);
        this.textLabels[idx].position.y += this.physics.sizeData[idx] + 1.2;
        // Make text always face the camera (will be set in createBallpit)
        if (this.camera) {
          this.textLabels[idx].lookAt(this.camera.position);
        }
      }
    }
...
function createBallpit(e, t = {}) {
  ...
  i.onBeforeRender = e => {
    if (!c) {
      s.camera = i.camera; // Set camera reference for text labels
      s.update(e);
    }
  };
  i.onAfterResize = e => {
